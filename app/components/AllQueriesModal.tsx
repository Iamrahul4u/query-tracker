          ...
          >
            {flowItems.map((item, index) => {
              if (item.type === "userHeader") {
                ...
                // Bucket header - main in user view, sub in type-first bucket view
                const isMainHeader = currentViewMode === "user";
                return (
                  <div
                    key={`header-${item.bucket}`}
                    className={`flex items-center justify-between rounded-lg mb-1 ${isMainHeader ? "px-3 py-2" : "px-2 py-1"}`}
                    style={{
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`rounded bg-white/20 flex items-center justify-center text-white font-bold ${isMainHeader ? "w-6 h-6 text-sm" : "w-4 h-4 text-[10px]"}`}
                      >
                        {item.bucket}
                      </span>
                      <span
                        className={`font-semibold text-white ${isMainHeader ? "text-sm" : "text-[11px]"}`}
                      >
                        {item.config.name.replace(`${item.bucket}) `, "")}
                      </span>
                    </div>
                    <span
                      className={`px-2 py-0.5 rounded-full font-bold bg-white/20 text-white ${isMainHeader ? "text-xs" : "text-[9px]"}`}
                    >
                      {item.count}
                    </span>
                  </div>
